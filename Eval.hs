...
--Allows me not to be kicked for saying lol while everyone else will. (Tricky tricky)
eval "FMKilo" _ "lol" = return ()
eval "FMKilo-d2usc" _ "lol" = return ()
eval "FMKilo-otter2-cm" _ "lol" = return ()
eval "FMKilo-mx1" _ "lol" = return ()
...
--Commands that do something other than play around with text
    ...
    | "!devoice" `isPrefixOf` x = write ("MODE #kf2-dev -v "++y) (drop 70000 x)
...
eval "FMKilo" _ x
    ...
    | "!opme" `isPrefixOf` x = write "MODE #kf2-dev +o FMKilo" (drop 70000 x)
...
eval "FMKilo-d2usc" _ x
    ...
    | "!opme" `isPrefixOf` x = write "MODE #kf2-dev +o FMKilo-d2usc" (drop 70000 x)
    | "!op " `isPrefixOf` x = write "MODE #kf2-dev +o " (drop 4 x)
    | "!deop " `isPrefixOf` x = write "MODE -o " (drop 6 x)
...
eval "FMKilo-otter2-cm" _ x
    ...
    | "!me " `isPrefixOf` x = privmsg ("\001ACTION "++(drop 4 x)++"\001")
    | "!part " `isPrefixOf` x = write "PART" (drop 6 x)
    | "!opme" `isPrefixOf` x = write "MODE #kf2-dev +o FMKilo-otter1-cm" (drop 70000 x)
    | "!op " `isPrefixOf` x = write "MODE #kf2-dev +o " (drop 4 x)
    | "!deop " `isPrefixOf` x = write "MODE -o " (drop 6 x)
...
eval "IngCr3at1on" _ x
    ...
    | "!msg " `isPrefixOf` x = write "PRIVMSG" (drop 5 x)
    | "!topic " `isPrefixOf` x = write ("PRIVMSG chanserv :topic "++chan) (drop 7 x)
    | "!opme" `isPrefixOf` x = write "MODE #kf2-dev +o IngCr3at1on" (drop 70000 x)
    | "!op " `isPrefixOf` x = write "MODE #kf2-dev +o " (drop 4 x)
    | "!deop " `isPrefixOf` x = write "MODE -o " (drop 6 x)
